...
Usage:
    python -m tqbench.benchmarks.embeddings.runners.run_all                  # full pipeline
    python -m tqbench.benchmarks.embeddings.runners.run_all --skip-phase2    # quality only
    python -m tqbench.benchmarks.embeddings.runners.run_all --models qwen3-embedding-8b-int8 harrier-oss-0.6b-bf16
"""
...
def main() -> None:
    ...
    _run(sys.executable, "-m", "tqbench.benchmarks.embeddings.runners.build_layer2", "--skip-2b")
    _run(sys.executable, "-m", "tqbench.benchmarks.embeddings.runners.build_layer2b", "--n-chunks", "1500")
    _run(sys.executable, "-m", "tqbench.benchmarks.embeddings.runners.build_adversarial")
    _run(sys.executable, "-m", "tqbench.benchmarks.embeddings.runners.build_dirty")

    if not args.skip_phase0:
        _run(sys.executable, "-m", "tqbench.benchmarks.embeddings.runners.smoke_test_vllm")
    ...
    if not args.skip_phase1_embed:
        for m in models:
            ...
            if elapsed_h > PHASE1_TIMEBOX_HOURS:
                ...
                break
            _run(sys.executable, "-m", "tqbench.benchmarks.embeddings.runners.embed_corpus", "--model", m)

    # Pool-and-judge Layer 1 after embeddings exist
    _run(sys.executable, "-m", "tqbench.benchmarks.embeddings.runners.build_layer1")

    for m in models:
        _run(sys.executable, "-m", "tqbench.benchmarks.embeddings.runners.eval_quality", "--model", m)
    ...
    if not args.skip_phase2:
        ...
        for m in top_two:
            ...
            if spec.kind == "api":
                continue
            _run(sys.executable, "-m", "tqbench.benchmarks.embeddings.runners.eval_speed", "--model", m)

    _run(sys.executable, "-m", "tqbench.benchmarks.embeddings.runners.build_report")
